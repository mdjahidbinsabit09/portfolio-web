...
const emptyForm = { title: '', company: '', duration: '' };
...
export default function ExperiencePage() {
  ...
  const startEdit = (item) => {
    setEditId(item._id);
    setEditForm({ title: item.title || '', company: item.company || '', duration: item.duration || '' });
    setShowAddForm(false);
  };
  ...
  const handleUpdate = async (e) => {
    ...
    setSaving(true);
    try {
      const res = await fetch(`/api/admin/experience/${editId}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
  ...
  const handleDelete = async () => {
    ...
    setDeleting(true);
    try {
      const res = await fetch(`/api/admin/experience/${deleteTarget._id}`, {
        method: 'DELETE',
      });
      if (!res.ok) {
        const data = await res.json().catch(() => ({}));
  ...
  if (loading) {
    return (
      <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', minHeight: '40vh' }}>
        <div style={{ color: 'var(--text-secondary)', fontSize: '0.95rem' }}>Loading experience...</div>
      </div>
    );
  }
  ...
  return (
    <div style={{ maxWidth: '800px' }}>
      ...
      <DeleteModal
        ...
        title={deleteTarget?.title || 'experience'}
        loading={deleting}
      />
    </div>
  );
